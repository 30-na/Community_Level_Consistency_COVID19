...
consis_4week_LMH_MT_suppNA_b5 = consis %>%
    arrange(date) %>%
...
consis_4week_total_LMH_MT_suppNA_b5 = consis %>%
    arrange(date) %>%
    group_by(date) %>%
    summarize(consis_counties = sum(consis_4weeks, na.rm = TRUE),
              total_counties = sum(!is.na(consis_4weeks)),
...
fig_consis_4week_line_LMH_MT_suppNA_b5 = ggplot(data = consis_4week_LMH_MT_suppNA_b5,
                                             aes(x = date,
                                                 y = consisRate,
    ...
    geom_smooth(method = "lm",
                ...
                size = 1.5)+
    geom_point(alpha = .3)+
    labs(title = "A) 4-week community risk level consistency rates with alternative \nthreshold for low risk level\n",
         x = NULL,
         y = "Consistency Rate")+
...
means = aggregate(consisRate ~  community_level,
                  consis_4week_LMH_MT_suppNA_b5,
                  mean) %>%
    mutate(consisRate = round(consisRate, 2))

fig_consis_4week_box_LMH_MT_suppNA_b5 = ggplot(data = consis_4week_LMH_MT_suppNA_b5,
...
fig_consis_4week_total_line_LMH_MT_suppNA_b5 = ggplot(consis_4week_total_LMH_MT_suppNA_b5, aes(x=date,
                                                                                         y=consisRate))+
    geom_smooth(method = "lm",
                formula = y ~ poly(x, 21))+
    geom_hline(yintercept = mean(consis_4week_total_LMH_MT_suppNA_b5$consisRate),
               linetype = "dashed")+
    geom_point(alpha = .5)+
    theme_bw()+
...
fig_consis_4week_total_box_LMH_MT_suppNA_b5 = ggplot(consis_4week_total_LMH_MT_suppNA_b5, aes(y=consis_4weeks, x=consisRate))+
...
fig_county_4week_proportion_line_LMH_MT_suppNA_b5 = community_level_LMH_MT_suppNA_b5 %>%
    group_by(date)%>%
    mutate(community_level = factor(x = community_level,
                                    levels = c("High", "Medium", "Low"),
                                    labels = c("High", "Medium", "Low")))%>%
...
save(consis_4week_LMH_MT_suppNA_b5,
     consis_4week_total_LMH_MT_suppNA_b5,
     fig_consis_4week_line_LMH_MT_suppNA_b5,
     fig_consis_4week_box_LMH_MT_suppNA_b5,
     fig_consis_4week_total_line_LMH_MT_suppNA_b5,
     fig_consis_4week_total_box_LMH_MT_suppNA_b5,
     fig_county_4week_proportion_line_LMH_MT_suppNA_b5,
     file = "Result/consistency_4week_low_medium_high_MT_suppNA_b5.Rda")


compare_consisRate_4week_LMH_MT_suppNA_b5 = grid.arrange(fig_consis_4week_line_LMH_MT_suppNA_b5,
                                                      fig_consis_4week_box_LMH_MT_suppNA_b5,
                                                      nrow = 1,
                                                      ncol = 3,
                                                      layout_matrix = rbind(c(1,1,2)))

ggsave("Result/Figures/compare_consisRate_4week_LMH_MT_suppNA_b5.jpg",
       compare_consisRate_4week_LMH_MT_suppNA_b5, 
