...
community_level_stateFips = community_level_LMH_suppNA %>%
...
consis_3week_LMH_suppNA = consis %>%
    arrange(date) %>%
...
consis_3week_total_LMH_suppNA = consis %>%
    arrange(date) %>%
    group_by(date) %>%
    summarize(consis_counties = sum(consis_3weeks, na.rm = TRUE),
              total_counties = sum(!is.na(consis_3weeks)),
...
fig_consis_3week_line_LMH_suppNA = ggplot(data = consis_3week_LMH_suppNA,
                                   aes(x = date,
                                       y = consisRate,
...
means = aggregate(consisRate ~  community_level,
                  consis_3week_LMH_suppNA,
                  mean) %>%
    mutate(consisRate = round(consisRate, 2))



fig_consis_3week_box_LMH_suppNA = ggplot(data = consis_3week_LMH_suppNA,
...
fig_consis_3week_total_line_LMH_suppNA = ggplot(consis_3week_total_LMH_suppNA, aes(x=date,
                                                                     y=consisRate))+
    geom_smooth(method = "lm",
                formula = y ~ poly(x, 21),
                color = "#8da0cb")+
    geom_hline(yintercept = mean(consis_3week_total_LMH_suppNA$consisRate),
               linetype = "dashed")+
    geom_point(alpha = .5)+
    theme_bw()+
...
fig_consis_3week_total_box_LMH_suppNA = ggplot(consis_3week_total_LMH_suppNA, aes(y=consis_3weeks, x=consisRate))+
...
fig_county_3week_proportion_line_LMH_suppNA = community_level_LMH_suppNA %>%
    group_by(date)%>%
    mutate(community_level = factor(x = community_level,
                                    levels = c("High", "Medium", "Low"),
                                    labels = c("High", "Medium", "Low")))%>%
...
save(consis_3week_LMH_suppNA,
     consis_3week_total_LMH_suppNA,
     fig_consis_3week_line_LMH_suppNA,
     fig_consis_3week_box_LMH_suppNA,
     fig_consis_3week_total_line_LMH_suppNA,
     fig_consis_3week_total_box_LMH_suppNA,
     fig_county_3week_proportion_line_LMH_suppNA,
     file = "Result/consistency_3week_low_medium_high_suppNA.Rda")

compare_consisRate_3week_LMH_suppNA = grid.arrange(fig_consis_3week_line_LMH_suppNA,
                                            fig_consis_3week_box_LMH_suppNA,
                                            nrow = 1,
                                            ncol = 3,
                                            layout_matrix = rbind(c(1,1,2)))

ggsave("Result/Figures/compare_consisRate_3week_LMH_suppNA.jpg",
       compare_consisRate_3week_LMH_suppNA, 
