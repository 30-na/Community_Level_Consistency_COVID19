...
# merge all datasest
merged_changeRate_LMH_supp1 = changeProb_calculate %>%
...
fig_NCHS_probChange_box_LMH_supp1 = ggplot(data = merged_changeRate_LMH_supp1,
                                 mapping = aes(x = UR_category,
                                               y = prob_risk_changed,
                                               fill = UR_category)) +
    geom_boxplot(alpha = .7)+
    #geom_smooth(method = "loess") +
    labs(title="\n \n rate of change in different NCHS Urban-Rural Category (suppressed = 1)",
         y = "rate of Change") +
    theme_bw()+
...
ggsave("Result/Figures/fig_NCHS_probChange_box_LMH_supp1.jpg",
       fig_NCHS_probChange_box_LMH_supp1, height=4,width=8,scale=1.65)
