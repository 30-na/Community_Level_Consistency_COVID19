...
fig_consis_2week_line_MH = ggplot(data = consis_2week_MH,
                                       aes(x = date,
                                           y = consisRate,
    ...
    labs(title="B) 2-week community risk level consistency rate while merging high and medium risk groups",
         x = NULL,
         y = "Consistency Rate")+
    theme_classic()+
...
fig_consis_2week_box_MH = ggplot(data = consis_2week_MH,
    ...
    labs(title = "E) 2-week community risk level consistency rates \n while merging high and medium risk groups",
...
fig_consis_2week_total_line_MH = ggplot(consis_2week_total_MH, aes(x=date,
                                                                        y=consisRate))+
    geom_smooth(method = "lm",
                formula = y ~ poly(x, 21),
                color = "#66c2a5")+
    geom_hline(yintercept=mean(consis_2week_total_MH$consisRate),
               linetype="dashed")+
    geom_point(alpha = .5)+
    theme_bw()+
...
fig_consis_2week_total_box_MH = ggplot(consis_2week_total_MH, aes(y=consis_2weeks, x=consisRate))+
...
save(consis_2week_MH,
     consis_2week_total_MH,
     fig_consis_2week_line_MH,
     fig_consis_2week_box_MH,
     fig_consis_2week_total_line_MH,
     fig_consis_2week_total_box_MH,
     file = "Result/consistency_2week_merged_high_medium.Rda")
