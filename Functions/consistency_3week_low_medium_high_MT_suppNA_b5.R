...
# LOW MEDIUM HIGH 
# load dataset
load("Result/CDC_community_level_county_computed_low_medium_high_modified_threshold_suppNA_b5.RDA")

# list of all available weeks
days = sort(unique(community_level_LMH_MT_suppNA_b5$date))


# list of all available counties
community_level_stateFips = community_level_LMH_MT_suppNA_b5 %>%
...
consis_3week_LMH_MT_suppNA_b5 = consis %>%
    arrange(date) %>%
...
consis_3week_total_LMH_MT_suppNA_b5 = consis %>%
    arrange(date) %>%
    group_by(date) %>%
    summarize(consis_counties = sum(consis_3weeks, na.rm = TRUE),
              total_counties = sum(!is.na(consis_3weeks)),
...
fig_consis_3week_line_LMH_MT_suppNA_b5 = ggplot(data = consis_3week_LMH_MT_suppNA_b5,
                                             aes(x = date,
                                                 y = consisRate,
...
means = aggregate(consisRate ~  community_level,
                  consis_3week_LMH_MT_suppNA_b5,
                  mean) %>%
    mutate(consisRate = round(consisRate, 2))



fig_consis_3week_box_LMH_MT_suppNA_b5 = ggplot(data = consis_3week_LMH_MT_suppNA_b5,
    ...
    guides(fill=guide_legend(title="Community Level"))+
    labs(title = NULL,
         y = "Consistency Rate",x = "alternative 1") +
    scale_y_continuous(limits=c(0,1),
                       breaks=c(0, .25, .50, 0.75, 1),
                       expand = c(0, 0))+
    scale_x_discrete()
...
fig_consis_3week_total_line_LMH_MT_suppNA_b5 = ggplot(consis_3week_total_LMH_MT_suppNA_b5, aes(x=date,
                                                                                         y=consisRate))+
    geom_smooth(method = "lm",
                formula = y ~ poly(x, 21),
                color = "#8da0cb")+
    geom_hline(yintercept = mean(consis_3week_total_LMH_MT_suppNA_b5$consisRate),
               linetype = "dashed")+
    geom_point(alpha = .5)+
    theme_bw()+
    labs(title="A) Counties at all community risk levels (without suppressed)",
         x = "Date",
         y = "Consistency Rate")


### Total consistency Rate (Box plot)
fig_consis_3week_total_box_LMH_MT_suppNA_b5 = ggplot(consis_3week_total_LMH_MT_suppNA_b5, aes(y=consis_3weeks, x=consisRate))+
...
fig_county_3week_proportion_line_LMH_MT_suppNA_b5 = community_level_LMH_MT_suppNA_b5 %>%
    group_by(date)%>%
    mutate(community_level = factor(x = community_level,
                                    levels = c("High", "Medium", "Low"),
                                    labels = c("High", "Medium", "Low")))%>%
...
save(consis_3week_LMH_MT_suppNA_b5,
     consis_3week_total_LMH_MT_suppNA_b5,
     fig_consis_3week_line_LMH_MT_suppNA_b5,
     fig_consis_3week_box_LMH_MT_suppNA_b5,
     fig_consis_3week_total_line_LMH_MT_suppNA_b5,
     fig_consis_3week_total_box_LMH_MT_suppNA_b5,
     fig_county_3week_proportion_line_LMH_MT_suppNA_b5,
     file = "Result/consistency_3week_low_medium_high_MT_suppNA_b5.Rda")

compare_consisRate_3week_LMH_MT_suppNA_b5 = grid.arrange(fig_consis_3week_line_LMH_MT_suppNA_b5,
                                                      fig_consis_3week_box_LMH_MT_suppNA_b5,
                                                      nrow = 1,
                                                      ncol = 3,
                                                      layout_matrix = rbind(c(1,1,2)))

ggsave("Result/Figures/compare_consisRate_3week_LMH_MT_suppNA_b5.jpg",
       compare_consisRate_3week_LMH_MT_suppNA_b5, 
