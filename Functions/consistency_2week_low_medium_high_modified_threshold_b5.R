...
consis_2week_total_LMH_MT_b5 = consis %>%
    arrange(date) %>%
    group_by(date) %>%
    summarize(consis_counties = sum(consis_2weeks, na.rm = TRUE),
              total_counties = sum(!is.na(consis_2weeks)),
...
fig_consis_2week_line_LMH_MT_b5 = ggplot(data = consis_2week_LMH_MT_b5,
                                      aes(x = date,
                                          y = consisRate,
...
means = aggregate(consisRate ~  community_level,
                  consis_2week_LMH_MT_b5,
                  mean) %>%
    mutate(consisRate = round(consisRate, 2))



fig_consis_2week_box_LMH_MT_b5 = ggplot(data = consis_2week_LMH_MT_b5,
...
fig_consis_2week_total_line_LMH_MT_b5 = ggplot(consis_2week_total_LMH_MT_b5, aes(x=date,
                                                                           y=consisRate))+
    geom_smooth(method = "lm",
                formula = y ~ poly(x, 21))+
    geom_hline(yintercept = mean(consis_2week_total_LMH_MT_b5$consisRate),
               linetype = "dashed")+
    geom_point(alpha = .5)+
    theme_bw()+
...
fig_consis_2week_total_box_LMH_MT_b5 = ggplot(consis_2week_total_LMH_MT_b5, aes(y=consis_2weeks, x=consisRate))+
...
fig_county_2week_proportion_line_LMH_MT_b5 = community_level_LMH_MT_b5 %>%
    group_by(date)%>%
    mutate(community_level = factor(x = community_level,
                                    levels = c("High", "Medium", "Low"),
                                    labels = c("High", "Medium", "Low")))%>%
...
save(consis_2week_LMH_MT_b5,
     consis_2week_total_LMH_MT_b5,
     fig_consis_2week_line_LMH_MT_b5,
     fig_consis_2week_box_LMH_MT_b5,
     fig_consis_2week_total_line_LMH_MT_b5,
     fig_consis_2week_total_box_LMH_MT_b5,
     fig_county_2week_proportion_line_LMH_MT_b5,
     file = "Result/consistency_2week_low_medium_high_modified_threshold_b5.Rda")

compare_consisRate_2week_LMH_MT_b5 = grid.arrange(fig_consis_2week_line_LMH_MT_b5,
                                               fig_consis_2week_box_LMH_MT_b5,
                                               nrow = 1,
                                               ncol = 3,
                                               layout_matrix = rbind(c(1,1,2)))

ggsave("Result/Figures/compare_consisRate_2week_LMH_MT_b5.jpg",
       compare_consisRate_2week_LMH_MT_b5, 
