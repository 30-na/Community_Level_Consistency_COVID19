...
consis_2week_LMH_suppNA = consis %>%
    arrange(date) %>%
...
consis_2week_total_LMH_suppNA = consis %>%
    arrange(date) %>%
    group_by(date) %>%
    summarize(consis_counties = sum(consis_2weeks, na.rm = TRUE),
              total_counties = sum(!is.na(consis_2weeks)),
...
fig_consis_2week_line_LMH_suppNA = ggplot(data = consis_2week_LMH_suppNA,
                                   aes(x = date,
                                       y = consisRate,
    ...
    geom_smooth(method = "lm",
                ...
                size = 1.5)+
    geom_point(alpha = .3)+
    labs(title = "A) 2-week community risk level consistency rates using \ncurrent CDC guidelines",
         x = NULL,
         y = "Consistency Rate")+
...
means = aggregate(consisRate ~  community_level,
                  consis_2week_LMH_suppNA,
                  mean) %>%
    mutate(consisRate = round(consisRate, 2))


fig_consis_2week_box_LMH_suppNA = ggplot(data = consis_2week_LMH_suppNA,
...
fig_consis_2week_total_line_LMH_suppNA = ggplot(consis_2week_total_LMH_suppNA, aes(x=date,
                                                                     y=consisRate))+
    geom_smooth(method = "lm",
                formula = y ~ poly(x, 21),
                color = "#8da0cb")+
    geom_hline(yintercept = mean(consis_2week_total_LMH_suppNA$consisRate),
               linetype = "dashed")+
    geom_point(alpha = .5)+
    theme_bw()+
...
fig_consis_2week_total_box_LMH_suppNA = ggplot(consis_2week_total_LMH_suppNA, aes(y=consis_2weeks, x=consisRate))+
...
fig_county_2week_proportion_line_LMH_suppNA = community_level_LMH_suppNA %>%
    group_by(date)%>%
    mutate(community_level = factor(x = community_level,
                                    levels = c("High", "Medium", "Low"),
                                    labels = c("High", "Medium", "Low")))%>%
...
save(consis_2week_LMH_suppNA,
     consis_2week_total_LMH_suppNA,
     fig_consis_2week_line_LMH_suppNA,
     fig_consis_2week_box_LMH_suppNA,
     fig_consis_2week_total_line_LMH_suppNA,
     fig_consis_2week_total_box_LMH_suppNA,
     fig_county_2week_proportion_line_LMH_suppNA,
     file = "Result/consistency_2week_low_medium_high_suppNA.Rda")


compare_consisRate_2week_LMH_suppNA = grid.arrange(fig_consis_2week_line_LMH_suppNA,
                                            fig_consis_2week_box_LMH_suppNA,
                                            nrow = 1,
                                            ncol = 3,
                                            layout_matrix = rbind(c(1,1,2)))

ggsave("Result/Figures/compare_consisRate_2week_LMH_suppNA.jpg",
       compare_consisRate_2week_LMH_suppNA, 
