...
fig_consis_2week_line_LMH = ggplot(data = consis_2week_LMH,
                                  aes(x = date,
                                      y = consisRate,
...
fig_consis_2week_total_line_LMH = ggplot(consis_2week_total_LMH, aes(x=date,
                                                                    y=consisRate))+
    geom_smooth(method = "lm",
                formula = y ~ poly(x, 21),
                color = "#8da0cb")+
    geom_hline(yintercept = mean(consis_2week_total_LMH$consisRate),
               linetype = "dashed")+
    geom_point(alpha = .5)+
    theme_bw()+
...
fig_consis_2week_total_box_LMH = ggplot(consis_2week_total_LMH, aes(y=consis_2weeks, x=consisRate))+
...
fig_county_2week_proportion_line_LMH = community_level_LMH %>%
    group_by(date)%>%
    mutate(community_level = factor(x = community_level,
                                    levels = c("High", "Medium", "Low"),
                                    labels = c("High", "Medium", "Low")))%>%
...
save(consis_2week_LMH,
     consis_2week_total_LMH,
     fig_consis_2week_line_LMH,
     fig_consis_2week_box_LMH,
     fig_consis_2week_total_line_LMH,
