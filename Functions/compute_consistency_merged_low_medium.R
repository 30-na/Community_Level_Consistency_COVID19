...
############################### Part II LOW AND MEDIUM MERGED############################
load("Result/CDC_community_level_county_computed_merged_Medium_With_Low.RDA")
...
fig_consis_rate_total_line_LM = ggplot(consis_plot_3_total, aes(x=date,
                                                                y=consisRate))+
    geom_smooth(method = "lm",
                formula = y ~ poly(x, 21))+
    geom_point(alpha = .5)+
...
fig_consisRate_box_LM = ggplot(consis_plot_3_total, aes(y=consis_3weeks, x=consisRate))+
...
save(fig_consis_rate_line02_LM,
