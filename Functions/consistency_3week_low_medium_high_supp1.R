...
community_level_stateFips = community_level_LMH_supp1 %>%
...
consis_3week_total_LMH_supp1 = consis %>%
    arrange(date) %>%
    group_by(date) %>%
    summarize(consis_counties = sum(consis_3weeks, na.rm = TRUE),
              total_counties = sum(!is.na(consis_3weeks)),
...
fig_consis_3week_line_LMH_supp1 = ggplot(data = consis_3week_LMH_supp1,
                                          aes(x = date,
                                              y = consisRate,
...
means = aggregate(consisRate ~  community_level,
                  consis_3week_LMH_supp1,
                  mean) %>%
    mutate(consisRate = round(consisRate, 2))


fig_consis_3week_box_LMH_supp1 = ggplot(data = consis_3week_LMH_supp1,
...
fig_consis_3week_total_line_LMH_supp1 = ggplot(consis_3week_total_LMH_supp1, aes(x=date,
                                                                                   y=consisRate))+
    geom_smooth(method = "lm",
                formula = y ~ poly(x, 21),
                color = "#8da0cb")+
    geom_hline(yintercept = mean(consis_3week_total_LMH_supp1$consisRate),
               linetype = "dashed")+
    geom_point(alpha = .5)+
    theme_bw()+
    labs(title="A) Counties at all community risk levels (suppressed = 1)",
         x = "Date",
         y = "Consistency Rate")


### Total consistency Rate (Box plot)
fig_consis_3week_total_box_LMH_supp1 = ggplot(consis_3week_total_LMH_supp1, aes(y=consis_3weeks, x=consisRate))+
...
fig_county_3week_proportion_line_LMH_supp1 = community_level_LMH_supp1 %>%
    group_by(date)%>%
    mutate(community_level = factor(x = community_level,
                                    levels = c("High", "Medium", "Low"),
                                    labels = c("High", "Medium", "Low")))%>%
...
save(consis_3week_LMH_supp1,
     consis_3week_total_LMH_supp1,
     fig_consis_3week_line_LMH_supp1,
     fig_consis_3week_box_LMH_supp1,
     fig_consis_3week_total_line_LMH_supp1,
     fig_consis_3week_total_box_LMH_supp1,
     fig_county_3week_proportion_line_LMH_supp1,
     file = "Result/consistency_3week_low_medium_high_supp1.Rda")

compare_consisRate_3week_LMH_supp1 = grid.arrange(fig_consis_3week_line_LMH_supp1,
                                                   fig_consis_3week_box_LMH_supp1,
                                                   nrow = 1,
                                                   ncol = 3,
                                                   layout_matrix = rbind(c(1,1,2)))

ggsave("Result/Figures/compare_consisRate_3week_LMH_supp1.jpg",
       compare_consisRate_3week_LMH_supp1, 
